...
export const invalid = async (req, res) => {
  ...
  const user = await User.findOne({ userID: userID });
  const itemsTakenByUser = user.itemsTakenFromInventory;
  ...
  try {
    for (const item of items) {
      ...
      console.log("NEW QUANTITTY", newquantity);
      ...
      if (foundItem && foundItem.quantity < quantity) {
        ...
        const name = newquantity.itemName;
        const itemToUpdateIndex = itemsTakenByUser.findIndex(
          (item) => item.item_id == itemID
        );
        ...
        if (itemToUpdateIndex !== -1) {
          ...
          await User.findOneAndUpdate(
            { userID: userID },
            { itemsTakenFromInventory: itemsTakenByUser }
          );
        } else {
          const itemToAdd = {
            item_id: itemID,
        ...
        console.log("Less than requested");
